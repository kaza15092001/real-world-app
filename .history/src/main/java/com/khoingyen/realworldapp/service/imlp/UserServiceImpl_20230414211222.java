...
public class UserServiceImpl implements UserService {
    ...
    @Override
    public Map<String, UserDTOResponse> login(Map<String, UserDTORequestLogin> userRequestLoginMap)
        ...
        if (userOptional.isPresent()) {
            ...
            if (passwordEncoder.matches(userDTORequestLogin.getPassword(), user.getPassword()))
                isAuthentication = true;
        }
        if (!isAuthentication) {
            throw new CustomNotFoundException(
                    CustomError.builder().code("404").message("Email or Password is incorrect").build());
        }
    ...
    public Map<String, UserDTOResponse> updateUser(Map<String, UserDTOUpdate> userDTOUpdateMap) {
        ...
        User user = UserMapper.toUser(userDTOUpdate, userLoggedIn.getId());
        user = userRepository.save(user);
        return Us;
    }
}
